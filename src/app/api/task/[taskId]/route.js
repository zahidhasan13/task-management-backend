...
export async function GET(req, { params }) {
  try {
    ...
    const { teamId } = await params; // fetch tasks by team
    ...
    // Fetch all tasks for the team, sorted newest first
    const tasks = await TaskSchema.find({ team: teamId })
...
export async function PATCH(req, { params }) {
  try {
    ...
    // - Assigned user can update only the status
    const isCreator = task.createdBy.toString() === decoded.id;
    const isAssignee = task.assignedTo?.toString() === decoded.id;
    ...
    const allowedFields = isCreator
      ...
      : ["status"]; // assigned person can only change status

    const invalidFields = [];

    Object.keys(updates).forEach((field) => {
      if (!allowedFields.includes(field)) {
        invalidFields.push(`Field '${field}' cannot be updated by you`);
      } else {
        // Basic validation
        if (
          field === "title" &&
          ...
          field === "assignedTo" &&
          !/^[0-9a-fA-F]{24}$/.test(updates[field])
        ) {
          ...
          field === "priority" &&
          !["low", "medium", "high", "critical"].includes(updates[field])
        ) {
          invalidFields.push(
